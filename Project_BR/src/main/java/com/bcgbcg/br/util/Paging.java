...
public class Paging {
	...
	public static String getPaging(String path, int nowPage, int recordPerPage, int totalRecord) {
		...
		int pagePerBlock = 10; 		// 한 블록당 표시할 페이지 개수
	...
	public static String getPaging(String path, String pathTail, int nowPage, int recordPerPage, int totalRecord) {
		...
		if(beginPageOfBlock < pagePerBlock) {
			...
			sb.append("<a href='" + path + "?currentPage=" + (beginPageOfBlock - 1) + pathTail + "'>◀</a>&nbsp;&nbsp;");
		}
		// 페이지 번호 표시
		// 페이지 번호의 링크 필요 유무에 따라 if처리
		// 1. 페이지 번호의 링크가 필요없는 경우 : page == nowPage
		...
		for(int page = beginPageOfBlock; page <= endPageOfBlock; page++) {
			if(page == nowPage) {
				...
				//sb.append("<a href='" + path + "?currentPage=" + (page) + "'>" + page + "</a>&nbsp;");
				sb.append("<a href='" + path + "?currentPage=" + (page) + pathTail + "'>" + page + "</a>&nbsp;");
			}
		}
		// 다음 버튼 표시
		if(endPageOfBlock >= totalPage) {
			...
			sb.append("&nbsp;<a href='" + path + "?currentPage=" + (endPageOfBlock + 1) + pathTail + "'>▶</a>");
		}
		return sb.toString();
	}
}
