...
<script type="text/javascript">
	function checkPurchaseSearch(){
		var isSearch = "${sessionScope.search}";
		if(isSearch =="YES"){
			location.href="searchPurchase?key=${key}&value=${value}&currentPage=${currentPage}";
		}else{
			location.href="purchaseList?currentPage=${currentPage}";
		}
	}
...
<%@ include file="template/mainLeftNavPurchase.jsp" %>
	...
	<div class="col-md-12">
		<table class="information-detail-table">
			<tr>
				<td class="attr-title">교환번호</td>
				<td>${pDto.pIdx }</td>
			</tr>
			<tr>
				<td class="attr-title">유저ID</td>
				<td>${pDto.uId_ }</td>
			</tr>
			<tr>
				<td class="attr-title">물품번호</td>
				<td>${pDto.gIdx }</td>
			</tr>
			<tr>
				<td class="attr-title">물품명</td>
				<td>${gDto.gName }</td>
			</tr>
			<tr>
				<td class="attr-title">교환 수량</td>
				<td>${pDto.pQuantity }</td>
			</tr>
			<tr>
				<td class="attr-title">사용 포인트</td>
				<td>${pDto.pQuantity * gDto.gPrice }</td>
			</tr>
			<tr>
				<td class="attr-title">받는 이</td>
			...
			<tr>
				<td class="attr-title">교환일</td>
				<td>${pDto.pDate }</td>
			</tr>
			<tr>
				<td class="attr-title">운송장번호</td>
				<td>
				<c:choose>
					<c:when test="${pDto.pInvoiceNo eq '' }">
						<form action="setInvoiceNo">
							<input type="hidden" name="pIdx" value="${pDto.pIdx }"/>
							<input type="hidden" name="currentPage" value="${currentPage }"/>
							<input type="text" name="pInvoiceNo"/>
							<!-- <input type="submit" value="운송장 등록"/> -->
							<button type="submit" class="btn btn-sm btn-info"><img src="resources/admin/img/icon-check-75.png"/>운송장 등록</button>
						</form>
					</c:when>
					<c:otherwise>
						${pDto.pInvoiceNo }<br/>
						<!-- <input type="button" name="searchNoBtn" id="searchNoBtn" value="운송장조회"/> -->
						<button type="button" class="btn btn-sm btn-info" name="searchNoBtn" id="searchNoBtn"><img src="resources/admin/img/icon-truck.png"/>운송장 조회</button>
					</c:otherwise>
				</c:choose>
				</td>
			</tr>
		...
		<br/>
		<div class="btn-box">
			<!-- <input type="button" value="목록으로 이동" onclick="checkPurchaseSearch()"/> -->
			<a href='javascript:void(0);' onclick="checkPurchaseSearch();" class="btn btn-primary btn-warning"><span class="glyphicon glyphicon-list"></span> 목록</a>
		</div>
		<br/>
		<hr/>
		<br/>
